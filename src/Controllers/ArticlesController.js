...
exports.getArticlesByShelfId = async (req, res) => {
	...
	const articles = await ArticlesService.getArticlesByShelfId(
		req.body.shelf_id
	);
...
exports.addToShoppingCart = async (req, res) => {
	...
	try {
		...
		await ArticlesService.manageStock(
			req.body.quantity,
			req.body.article_id,
			"reserved",
			connection
		...
		let alreadyOrdered =
			await OrdersService.fixQuantityIfArticleAlreadyOrdered(
				req.session.user.id,
				req.body.article_id,
				req.body.quantity,
				discountId,
...
exports.createArticle = async (req, res) => {
	...
	const insertedArticleId = await ArticlesService.createArticle(
		articleObject
	);

	return res.json({
		status: `Success`,
		insertedArticleId: insertedArticleId
	});
};
