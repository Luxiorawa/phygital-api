...
exports.createDiscount = async (req, res) => {
	...
	if (
		...
		return !errors.isEmpty()
			...
			: res.status(422).json({
					status: "Failed",
					errors: "missing a category_id or article_id"
			  });
	}
	...
	return res.json({
		status: `Success`,
		insertedDiscountId: insertedDiscountId
	});
};
...
exports.updateDiscount = async (req, res) => {
	...
	return res.json({
		status: "Success",
		isDiscountUpdated: isDiscountUpdated
	});
};
...
exports.deleteDiscount = async (req, res) => {
	...
	if (isDiscountDeleted) {
		return res.json({
			status: "Success",
			isDiscountDeleted: isDiscountDeleted
		});
	} else {
		return res.json({
			status: "Failed",
			message: "No discount were deleted (ID not found)"
		});
	}
};
